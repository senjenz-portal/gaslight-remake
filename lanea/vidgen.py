#!/usr/bin/env python3
"""Lane A video generator: Scenario image->video for the living plate.

Seamless-loop trick: pass the SAME plate as `image` (first frame) AND
`lastFrameImage` so the clip returns to its own opening frame.

Usage:
  vidgen.py --name backdrop-loop --image PATH --outdir DIR --prompt "..." \
            [--model model_bytedance-seedance-2-0] [--duration 8] [--res 720p] \
            [--aspect adaptive] [--no-last-frame]
...
def main():
    ...
    if grok:
        ...
        if not a.no_last_frame:
            body["lastFrameImage"] = aid          # seamless-loop trick
    ...
    assets, jobinfo = poll(job_id, timeout_s=2400, interval=12)
    if not assets:
